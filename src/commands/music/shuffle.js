...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (player.queue.length < 2) {
      return replyError(interaction, 'Not enough tracks', 'There must be at least 2 tracks in the queue to shuffle.');
    }
    player.shuffle();
    return interaction.reply({
      embeds: [embeds.success('Shuffled', `Shuffled **${player.queue.length}** queued tracks.`)],
    });
  },
};
